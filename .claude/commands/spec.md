...
allowed-tools: Read, Write, Glob, Bash(git switch:*), Bash(git config:*)
---
...
- A safe git branch name not already taken (e.g. anthony/feat/new-heist-form)
...
3. `branch_name`  
   - Format: `<git_user>/feat/<feature_slug>`, following the branch naming convention in CLAUDE.md.
   - `<git_user>` is the local part of the current Git user's email (the text before `@`), lowercased. Get it by running `git config user.email` and stripping everything from `@` onward.
   - Example: if `git config user.email` is `anthony.dionise@gmail.com` and the slug is `card-component`, the branch is `anthony/feat/card-component`.
...
Before making any content, switch to a new Git branch using the `branch_name` derived from the `$ARGUMENTS`. If the branch name is already taken, then append a version number to it: e.g. `anthony/feat/card-component-01`
